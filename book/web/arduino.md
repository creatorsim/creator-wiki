...
<img src="image/arduino/arduinoSetup2.png" width="300">
...
1. **Setup**: Establish Internal LED as an output and button as input.
   ...
   * The interrupt mode, that cav be one of the following:| Mode      | Value | Usage                                                                                                             |
     | --------- | ----- | ----------------------------------------------------------------------------------------------------------------- |
...
* **Steps**:
  ...
  * 10kΩ resistence
  ...
  <img src="image/arduino/colorGuide.png" width="300">
...
<img src="image/arduino/setupCase5.png" width="500">
...
Next, we move on to the `loop_buttons` function, where we will:
    1.  Check if we have already gone through the entire list
...
```
    ...
    2.  If this is not the case, we scroll through the data to find the PIN number we want.
...
```
    ...
    3. Once the position has been obtained, we configure the button with INPUT_PULLUP mode, as they are buttons.
...
```
    ...
    4. As we indicated in the statement, we are going to use interrupts, so we have to assign them now knowing which button we are on (that is, we take advantage of the fact that the order of`button_handlers` is the same as the order followed by the button array).
...
```
    ...
    5. We add up a position and re-enter the loop
...
```
    ...
    6. If we have finished going through the list, we return to main by executing a ret (or a jr ra).
