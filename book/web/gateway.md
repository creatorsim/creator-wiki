...
1. Download our [OpenOCD scripts](https://creatorsim.github.io/creator/gateway/openocd.zip)
...
1. Set the `OPENOCD_SCRIPTS` environment variable:

```bash
  export OPENOCD_SCRIPTS="/full/path/to/openocd-esp32/share/openocd/scripts"
```

1. Execute the `openocd_start.sh` script with the type of device (e.g. `esp32c3`)
   ```bash
...
1. Download our [OpenOCD scripts](https://creatorsim.github.io/creator/gateway/openocd.zip)
2. Install and setup [Zadig](https://zadig.akeo.ie/)
3. List all the devices in `Options` > `List All Devices` and select `USB Jtag/serial debug unit (Interface 2)`
   ![Setting the device in Zadig](img/gateway/zadig_set_device.png)
4. Downgrade the driver
   ![Downgrading the driver](img/gateway/zadig_downgrade_driver.png)
5. Setup OpenOCD
6. Download [OpenOCD with ESP32 JTAG support v0.12.0-esp32-20241016](https://github.com/espressif/openocd-esp32/releases/tag/v0.12.0-esp32-20241016) (for your OS and architecture) and unzip it
7. Add the `bin\` folder to your PATH, e.g.:
   ``powershell set PATH=%PATH%;<openocd-esp32 path>\bin\ ``
8. Execute the `openocd_start.bat` script with the type of device (e.g. `esp32c3`)
   ```powershell
...
3. Download and unzip the [ESP32 gateway]({{book.repo}}/gateway/esp32.zip)
4. Install requirements
   1. Use install.sh script from ESP-IDF
...
9. Download and unzip the [SBC gateway]({{book.repo}}/gateway/sbc.zip) in the SBC
