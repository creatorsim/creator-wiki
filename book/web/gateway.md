...
2. Connect your device and check which port it belongs to. It typically resides in `/dev/`, e.g. `/dev/ttyUSB0`. You can quickly check it with `ls /dev/ttyUSB*` (Linux) or `ls /dev/cu.usbserial-*` (macOS).
  > [!IMPORTANT]
  > **If you're using macOS**, set up the [Remote Serial Port](https://docs.espressif.com/projects/esp-idf/en/stable/esp32/api-guides/tools/idf-docker-image.html#using-remote-serial-port):
  > 1. Download and unzip [esptool](https://github.com/espressif/esptool/releases)
...
> [!TIP]
> For boards without the secondary port on the board, but with JTAG support, you can wire a daughter board as such:
> ![](img/gateway/jtag-connection.png)
