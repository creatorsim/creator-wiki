...
![Arduino library functions](img/arduino/lib.png){ width="300" loading=lazy }
/// caption
Library functions available after loading an Arduino library.
///

!!! tip
    Except the `printf` function (which is excusive for Espressif Arduino devices) and `rgbLedWrite` (only used for built-in internal LEDS in ESP32-C6 and ESP32-H2) all the functions displayed can be found in [Arduino's documentation](https://docs.arduino.cc/language-reference/#functions) and in _Help_ → _Creatino Help_.
...
![Example template](img/arduino/template.png){ width="300" loading=lazy }
/// caption
Example template for new CREATino programs.
///
...
CREATOR now supports a graphic simulator using CREATOR executor. It can be accessed next to the Statics Menu when loaded Arduino Library

![Arduino Debug page](img/arduino/debugger/debugger.png){ width="800" loading=lazy }
/// caption
Arduino Debug page in the simulator.
///

### Available boards
At the moment, the user can access to ESP32C3-DevkitC-02 and ESP32-C6-DevKitC-1 graphic models inside the simulator.
...
| :---: | :---: |
| ![ESP32C3-DevKitC-02 board](img/arduino/debugger/esp32c3.png){ width="350" loading=lazy } | ![ESP32C6-DevKitC-1 board](img/arduino/debugger/esp32c6.png){ width="350" loading=lazy } |
| 15 general-purpose pins. | 22 general-purpose pins. |


### Debug panel
To help debugging and following the execution of Arduino functions, there is _Arduino Function Tracer_ Terminal, which shows the user what action has been done (function execution, text, interrupts warning...)

![Arduino Function Tracer terminal](img/arduino/debugger/arduino_debug.gif){ width="500" loading=lazy }
/// caption
Arduino Function Tracer terminal during program execution.
///
...
| :---: | :---: | :---: | :---: | :---: |
| ![Input mode](img/arduino/debugger/input.png){ width="200" loading=lazy } | ![Output mode](img/arduino/debugger/output.png){ width="200" loading=lazy } | ![Input pullup mode](img/arduino/debugger/input_pullup.png){ width="90" loading=lazy } | ![Input pulldown mode](img/arduino/debugger/input_pulldown.png){ width="90" loading=lazy } | ![Analog mode](img/arduino/debugger/analog.png){ width="100" loading=lazy } |
...
| :---: | :---: |
| ![Automatic change](img/arduino/debugger/changepin.gif){ width="100" loading=lazy } | ![Manual change](img/arduino/debugger/change_manual.gif){ width="100" loading=lazy } |
...
![High level interrupt triggered by a button](img/arduino/debugger/interrupt_button.gif){ width="500" loading=lazy }
/// caption
High level interrupt reproduced graphically in the simulator.
///

!!! example
    See Example 14: _High level interrupts example_ to try this feature
...
![Internal LED blinking](img/arduino/example1.png){ width="300" loading=lazy }
/// caption
Internal LED blinking on the ESP32-C3-DevKitC-02 board.
///
...
![Button and LED wiring](img/arduino/setup2.png){ width="500" loading=lazy }
/// caption
Button and LED circuit setup for Example 2.
///
...
![Example 2 program execution](img/arduino/example2.png){ width="500" loading=lazy }
/// caption
LED toggling through button interrupts.
///
...
#### Step 1: Save messages to print in memory
...
#### Step 2: Start terminal output and input.
...
#### Step 3: Check if the terminal is correctly open.
...
`read_num` reads a number from terminal. In this case, we use the `serial_parseInt` function to get a number per terminal.
...
![Serial input and output](img/arduino/example3.png){ width="500" loading=lazy }
/// caption
Text input and output using the serial terminal.
///
...
![LDR and LED circuit setup](img/arduino/setup4.png){ width="300" loading=lazy }
/// caption
Circuit setup with the LDR sensor and LED.
///
...
![Resistor color code guide](img/arduino/colorGuide.png){ width="500" loading=lazy }
/// caption
Resistor color code guide.
///

#### Step 1: Initialize LDR, pin, and serial output.
...
![Daytime running lights result](img/arduino/example4.png){ width="500" loading=lazy }
/// caption
LED lighting up when low light is detected.
///


### Example 5: Piano (Advanced)
...
![Piano breadboard setup](img/arduino/setup5.png){ width="600" loading=lazy }
/// caption
Breadboard setup with four buttons and a passive buzzer.
///

#### Step 1: Declaration of pins and values of musical notes
...
```asm
  ...
  # attachInterrupt(digitalPinToInterrupt(buttonPin[i]), handler[i], ON_LOW);
  li a2, 0x03
  addi sp, sp, -4
  sw ra,0(sp)
  jal ra, attachInterrupt 
...
![Arduino piano](img/arduino/example5.png){ width="500" loading=lazy }
/// caption
Finished piano playing musical notes with a buzzer.
///
